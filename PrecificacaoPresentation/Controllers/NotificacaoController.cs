...
namespace ERP_Condominios_Solution.Controllers
{
    public class NotificacaoController : Controller
    {
        ...
        public JsonResult GetNotificacaoNaoLida()
        {
            ...
            listaMaster = baseApp.GetNotificacaoNovas(usu.USUA_CD_ID, idAss).Where(x => x.NOTC_DT_VISTA == null & x.NOTC_DT_EMISSAO == DateTime.Today.Date).ToList();
        ...
        public ActionResult NovaNotificacaoClick()
        {
            ...
            listaMaster = baseApp.GetNotificacaoNovas(usu.USUA_CD_ID, idAss).Where(x => x.NOTC_DT_VISTA == null).ToList();
            if (listaMaster.Count == 1)
            {
                return Json(listaMaster.FirstOrDefault().NOTC_CD_ID);
            }
            else
            {
                return Json(0);
        ...
        public ActionResult VerNotificacao(Int32 id)
        {
            ...
            item.NOTC_IN_VISTA = 1;
            item.NOTC_DT_VISTA = DateTime.Now;
            objetoAntes = item;
        ...
        public ActionResult FiltrarNotificacao(NOTIFICACAO item)
        {
            try
            {
                ...
                Int32 volta = baseApp.ExecuteFilter(item.NOTC_NM_TITULO, item.NOTC_DT_EMISSAO, item.NOTC_TX_NOTIFICACAO, idAss, out listaObj);
        ...
        public ActionResult IncluirNotificacao()
        {
            ...
            NOTIFICACAO item = new NOTIFICACAO();
            NotificacaoViewModel vm = Mapper.Map<NOTIFICACAO, NotificacaoViewModel>(item);
            vm.NOTC_DT_EMISSAO = DateTime.Now;
            vm.NOTC_IN_ATIVO = 1;
            vm.ASSI_CD_ID = idAss;
        ...
        public ActionResult IncluirNotificacao(NotificacaoViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Cria pastas
                    String caminho = "/Imagens/" + idAss.ToString() + "/Notificacao/" + item.NOTC_CD_ID.ToString() + "/Anexos/";
                    ...
                    Session["ListaNotificacao"] = null;
                    Session["VoltaNotificacao"] = 1;
                    Session["IdNotificacaoVolta"] = item.NOTC_CD_ID;
        ...
        public ActionResult UploadFileNotificacaoGeral(HttpPostedFileBase file)
        {
            ...
            foto.NOTC_CD_ID = item.NOTC_CD_ID;
        ...
        public ActionResult GerarRelatorioLista()
        {
            ...
            foreach (NOTIFICACAO item in lista)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(item.NOTC_NM_TITULO, meuFont))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                ...
                table.AddCell(cell);
                if (item.NOTC_DT_VALIDADE != null)
                {
                    cell = new PdfPCell(new Paragraph(item.NOTC_DT_VALIDADE.ToShortDateString(), meuFont))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
            ...
            if (filtro != null)
            {
                if (filtro.NOTC_NM_TITULO != null)
                {
                    parametros += "Título: " + filtro.NOTC_NM_TITULO;
                    ja = 1;
                }
                if (filtro.NOTC_DT_EMISSAO != null)
                {
                    if (ja == 0)
                    {
                ...
                if (filtro.NOTC_TX_NOTIFICACAO != null)
                {
                    if (ja == 0)
                    {
                        parametros += "Conteúdo: " + filtro.NOTC_TX_NOTIFICACAO;
                        ja = 1;
                    }
                    else
                    {
                        parametros += " e Conteúdo: " + filtro.NOTC_TX_NOTIFICACAO;
                    }
                }
                if (ja == 0)
                {
        ...
        public ActionResult GerarRelatorioDetalhe()
        {
            ...
            String data = DateTime.Today.Date.ToShortDateString();
            data = data.Substring(0, 2) + data.Substring(3, 2) + data.Substring(6, 4);
            String nomeRel = "Notificacao_" + aten.NOTC_CD_ID.ToString() + "_" + data + ".pdf";
            Font meuFont = FontFactory.GetFont("Arial", 8, iTextSharp.text.Font.NORMAL, BaseColor.BLACK);
            Font meuFont1 = FontFactory.GetFont("Arial", 9, iTextSharp.text.Font.NORMAL, BaseColor.BLACK);
            Font meuFont2 = FontFactory.GetFont("Arial", 12, iTextSharp.text.Font.NORMAL, BaseColor.BLACK);
            ...
            cell = new PdfPCell(new Paragraph("Título: " + aten.NOTC_NM_TITULO, meuFont));
            cell.Border = 0;
            cell.Colspan = 4;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            if (aten.NOTC_DT_VALIDADE != null)
            {
                cell = new PdfPCell(new Paragraph("Validade: " + aten.NOTC_DT_VALIDADE.ToShortDateString(), meuFont));
                cell.Border = 0;
                cell.Colspan = 1;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            if (aten.NOTC_DT_VISTA != null)
            {
                cell = new PdfPCell(new Paragraph("Data de Visualização: " + aten.NOTC_DT_VISTA.Value.ToShortDateString(), meuFont));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            cell = new PdfPCell(new Paragraph("Conteúdo: " + aten.NOTC_TX_NOTIFICACAO, meuFont));
            cell.Border = 0;
            cell.Colspan = 4;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
