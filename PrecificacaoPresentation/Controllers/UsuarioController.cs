...
namespace ERP_Condominios_Solution.Controllers
{
    public class UsuarioController : Controller
    {
        ...
        public UsuarioController(IUsuarioAppService baseApps, ILogAppService logApps, INotificacaoAppService notiApps, IConfiguracaoAppService confApps, IEmpresaAppService filApps)
        {
            baseApp = baseApps;
            logApp = logApps;
            notiApp = notiApps;
        ...
        public ActionResult FiltrarUsuario(USUARIO item)
        {
            try
            {
                ...
                Int32 volta = baseApp.ExecuteFilter(item.PERF_CD_ID, item.CARG_CD_ID, item.USUA_NM_NOME, item.USUA_NM_LOGIN, item.USUA_EM_EMAIL, idAss, out listaObj);
        ...
        public ActionResult EditarUsuario(Int32 id)
        {
            ...
            //List<SelectListItem> bloqueado = new List<SelectListItem>();
            //bloqueado.Add(new SelectListItem() { Text = "Ativa", Value = "Ativa" });
            //bloqueado.Add(new SelectListItem() { Text = "Inativa", Value = "Inativa" });
            //ViewBag.Bloqueado = new SelectList(bloqueado, "Value", "Text");
       ...
       [HttpGet]
        public ActionResult BloquearUsuario(Int32 id)
        {
            ...
            block.USUA_EM_EMAIL = item.USUA_EM_EMAIL;
        ...
        public ActionResult DesbloquearUsuario(Int32 id)
        {
            ...
            unblock.USUA_EM_EMAIL = item.USUA_EM_EMAIL;
        ...
        public ActionResult DesativarUsuario(Int32 id)
        {
            ...
            dis.USUA_EM_EMAIL = item.USUA_EM_EMAIL;
       ...
       [HttpPost]
        ...
        {            
            ...
            foreach (USUARIO item in lista)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(item.USUA_EM_EMAIL, meuFont))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE, HorizontalAlignment = Element.ALIGN_LEFT
                };
                table.AddCell(cell);
            ...
            if (filtro != null)
            {
                ...
                if (filtro.USUA_EM_EMAIL != null)
                {
                    if (ja == 0)
                    {
                        parametros += "E-Mail: " + filtro.USUA_EM_EMAIL;
                        ja = 1;
                    }
                    else
                    {
                        parametros += " e E-Mail: " + filtro.USUA_EM_EMAIL;
                    }
                }
        ...
        {            
            ...
            cell = new PdfPCell(new Paragraph("E-Mail: " + aten.USUA_EM_EMAIL, meuFont));
            cell.Border = 0;
            cell.Colspan = 3;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            cell = new PdfPCell(new Paragraph("Cargo: " + aten.CARGO_USUARIO.CARG_NM_NOME, meuFont));
            cell.Border = 0;
            cell.Colspan = 1;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            cell = new PdfPCell(new Paragraph("Acessos: " + CrossCutting.Formatters.DecimalFormatter(aten.USUA_NR_ACESSOS), meuFont));
            cell.Border = 0;
            cell.Colspan = 1;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            cell = new PdfPCell(new Paragraph("Falhas de Login: " + CrossCutting.Formatters.DecimalFormatter(aten.USUA_NR_FALHAS), meuFont));
            cell.Border = 0;
            cell.Colspan = 1;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
        ...
        public ActionResult MontarTelaDashboardAdministracao()
        {
            ...
            // Resumo Log Diario
            List<DateTime> datasCR = listaLog.Where(m => m.LOG_DT_LOG != null & m.LOG_DT_LOG.Month == DateTime.Today.Date.Month & m.LOG_DT_LOG.Year == DateTime.Today.Date.Year).Select(p => p.LOG_DT_LOG.Date).Distinct().ToList();
        ...
        public ActionResult EnviarEMailUsuario(Int32 id)
        {
            ...
            mens.ID = id;
            mens.MODELO = cont.USUA_EM_EMAIL;
        ...
        public Int32 ProcessaEnvioEMailUsuario(MensagemViewModel vm, USUARIO usuario)
        {
            ...
            mensagem.ASSUNTO = "Contato Usuário";
            mensagem.CORPO = emailBody;
            mensagem.DEFAULT_CREDENTIALS = false;
            mensagem.EMAIL_DESTINO = cont.USUA_EM_EMAIL;
