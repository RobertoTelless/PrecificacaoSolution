    ...
    public class TarefaController : Controller
    {
        ...
        public JsonResult GetTarefas()
        {
            ...
            foreach (var item in listaMaster)
            {
                ...
                hash.Add("TARE_DT_ESTIMADA", item.TARE_DT_ESTIMADA.ToString("dd/MM/yyyy"));
        ...
        public ActionResult FiltrarTarefa(TAREFA item)
        {
            ...
            try
            {
                ...
                Int32 volta = baseApp.ExecuteFilter(item.TITR_CD_ID, item.TARE_NM_TITULO, item.TARE_DT_CADASTRO, item.TARE_DT_ESTIMADA, item.TARE_IN_STATUS, item.TARE_IN_PRIORIDADE, item.USUA_CD_ID, idAss, out listaObj);
        ...
        public ActionResult GerarRelatorioLista()
        {
            ...
            foreach (TAREFA item in lista)
            {
                ...
                {
                    cell = new PdfPCell(new Paragraph(item.TARE_DT_ESTIMADA.ToShortDateString(), meuFont))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
